...
function updateUI(
  ...
  const emptyProductListMessage = document.getElementById(
  ...
  if (safeNewQty > 0) {
    if (!productRowLeft) {
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-4">
            <div class="w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl flex items-center justify-center group-hover:scale-105 transition-transform duration-200">
              <span class="text-white font-bold text-lg">${safeProductName
                .charAt(0)
                .toUpperCase()}</span>
            </div>
            <div>
      ...
      if (emptyProductListMessage)
        emptyProductListMessage.classList.add('hidden');
    } else {
    ...
    if (qtyInput) {
      console.log(
        `Updating quantity input from ${qtyInput.value} to ${safeNewQty}`
      );
    ...
    const subtotalElement = document.getElementById(
      `subtotal_${safeProductId}`
    );
    if (subtotalElement) {
      const subtotal = safeNewQty * safeProductPrice;
      console.log(
        `Updating subtotal: ${safeNewQty} x ${safeProductPrice} = ${subtotal}`
      );
      subtotalElement.textContent = formatRupiah(subtotal);
    }

    const stockStatusElement = document.getElementById(
      `stock-status-${safeProductId}`
    );
    if (stockStatusElement) {
      const remainingStock = safeProductStock - safeNewQty;
      const stockText =
        remainingStock > 0 ? `${remainingStock} tersisa` : 'Stok habis';
...
function updateCartUI(productId, productName, newQty, productPrice) {
  console.log(`updateCartUI called with:`, {
    productId,
    productName,
    newQty,
    productPrice,
  ...
  if (newQty > 0) {
    if (!cartItemDiv) {
      ...
      cartItemDiv.innerHTML = `
        <div class="flex items-center space-x-3">
          <div class="w-12 h-12 bg-gradient-to-br from-blue-100 to-indigo-100 rounded-xl flex items-center justify-center shadow-sm">
            <span class="text-blue-700 font-bold text-sm">${productName
              .charAt(0)
              .toUpperCase()}</span>
          </div>
          <div>
      ...
      const cartPriceElement = document.getElementById(
        `cart-price-${productId}`
      );

      if (cartQtyElement) {
        console.log(
          `Updating cart quantity from ${cartQtyElement.textContent} to ${newQty}`
        );
    ...
    if (cartSubtotalElement) {
      ...
      console.log(
        `Updating cart subtotal: ${newQty} x ${productPrice} = ${subtotal}`
      );
