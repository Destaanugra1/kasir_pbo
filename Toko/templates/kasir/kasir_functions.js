...
function updateTotalsAndCounts() {
  ...
  if (!daftarProdukList) {
    console.warn(
      "Element 'daftar-produk-list' not found when updating totals."
    );
    return;
  }
  ...
  const daftarProdukCountElement = document.getElementById(
    'daftar-produk-count'
  );
  ...
  if (totalPembayaranElement)
    totalPembayaranElement.textContent = formatRupiah(totalAmount);
  if (cartItemCountElement) cartItemCountElement.textContent = totalItemsInCart;
  if (daftarProdukCountElement)
    daftarProdukCountElement.textContent = totalItemsInCart;

  if (totalItemsInCart === 0) {
    if (emptyCartMessage) emptyCartMessage.classList.remove('hidden');
    if (checkoutSection) checkoutSection.classList.add('hidden');
    if (emptyProductListMessage)
      emptyProductListMessage.classList.remove('hidden');
  } else {
...
function updateTotalsDisplay(totalAmount, totalItems) {
  ...
  if (daftarProdukCountElement)
    daftarProdukCountElement.textContent = totalItems;
