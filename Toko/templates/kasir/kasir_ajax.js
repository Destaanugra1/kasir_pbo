...
async function updateQuantityAjax(productId, change, manualValue = null) {
  console.log(
    `updateQuantityAjax called with productId: ${productId}, change: ${change}, manualValue: ${manualValue}`
  );
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      document.getElementById(
        'qr-reader-result'
      ).textContent = `Gagal: ${data.message}`;
...
async function handleScannedBarcode(barcode) {
  document.getElementById('qr-reader-result').textContent =
    'Memproses barcode...';
  ...
  try {
    ...
    if (data.status === 'success') {
      ...
      document.getElementById(
        'qr-reader-result'
      ).textContent = `✅ ${productName} ditambahkan! Jumlah: ${newQtyInCart}`;
    } else {
      document.getElementById(
        'qr-reader-result'
